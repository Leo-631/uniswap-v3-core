...
contract UniswapV3Pool is IUniswapV3Pool, NoDelegateCall {
    //将库函数LowGasSafeMath附加到uint256，
    //uint256调用LowGasSafeMath重写的方法时，uint256的调用者会被作为第一个参数传入重写的方法中。
    //如uint256.add(x)相当于调用LowGasSafeMath.(uint256,x)
...
// 定义了结构体Slot0(当前最新price、当前tick、协议费用、池子状态(开启或关闭))、结构体ProtocolFees等：
    struct Slot0 {
        // the current price
    ...
    // 修饰器lock用于提供锁机制来规避重入攻击，此方法还可以防止在初始化池之前调用函数，
    // 重入保护在整个合约中是必需的，因为我们经常使用余额检查来确定交互(如铸币、掉期和闪存)的支付状态：
    modifier lock() {
...
// observe函数通过遍历请求参数获取每一个请求时间点的Oracle数据，数据获取通过observeSingle来实现：
    /// @inheritdoc IUniswapV3PoolDerivedState
...
// initialize函数用于对交易池进行初始化，主要涉及池子价格、trick、协议费用等，
// 这里的sqrtPriceX96为sqrt(amountToken1/amountToken0)Q64.96精度的定点数值，
// 在这里首先检查池子价格是否未初始化，之后调用getTickAtSqrtRatio函数以sqrtPriceX96为参数来计算最大的trick，
// 之后调用observations.initialize获取cardinality(基数)与cardinalityNext(下一个基数)的数值，
// 之后对slot0进行初始化操作，完成交易池的创建，当然此时交易池里面还没有流动性：
...
// 之后将其读入内存(Slot0 memory _slot0 = slot0;)，后续通过MLOAD直接访问而不用重新去加载LOAD，
// 从而节省gas，紧接着又调用了_updatePosition来更新position，这里我们先继续向下解读，
// 后面专门简介这一个函数，紧接着检查当前流动性的变更是否为0，如果不为0则检查当前的trick是否小于tricklower，
// 如果是则所有的token1将转变为token0，之后计算token0的变更，如果当前trick小于trickupper，
// 则通过observations.write增加Oracle条目，之后计算amout0和amount1的增量，最后当前trick范围内的流动性总量，
// 如果trick超过了trickupper则此时所有的token0将转变为token1，之后计算amount1的变更，
// 需要注意的是这里的amount0与amount1为int256类型，也就是说这里的amount0与amount1这两个返回值可正可负，
// 如果为正则表示流动性提供至需要给池子给予的数量，为负数则表示池子需要给流动性提供者给予的数量
...
// _updatePosition函数，在这里首先通过get函数来获取用户的position信息，之后检查流动性变更是否为0，
// 如果不为零则获取当前时间戳信息并通过observations.observeSingle来获取请求时间点的Oracle数据，
// 之后更新position对应的tricklower/trickupper的数据，之后检查flippedLower是否是第一次被引用，
// 或者移除了所有引用(移除流动性操作时)，如果是则更新trick位图，之后更新position数据，并清除之前的记录：
...
// StepComputations变量，该变量存储了交易过程中的信息：
...
// data：回调函数的参数
// 在这里首先检查交易数量是否为0，之后将交易前的数据保存在内存中，之后检查是否有正在进行的token交易操作，如果没有则根据交易方向检查价格是否满足在条件，
// 之后更新slot0.unlocked的状态，之后缓存交易的数据，之之后检查交换的数量是否大于0，并将其赋值于exactInput，之后存储交易状态信息：
    /// @inheritdoc IUniswapV3PoolActions
    function swap(
        address recipient,
        bool zeroForOne,
